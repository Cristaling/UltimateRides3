...
public class FileManager {
	...
	public Config getConfig(String name, String res) {
		if (!configs.containsKey(name))
			configs.put(name, new Config(name, res));
	...
	public class Config {
		...
		public Config(String name, String resource) {
			this.name = name;
		...
		/**
		 * Saves the default config(Will overwrite anything in the current
		 * config's file)
		 * <p>
		...
		public Config reload() {
			...
			try {
				defConfigStream = new InputStreamReader(plugin.getResource(this.resource), "UTF8");
